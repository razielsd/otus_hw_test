...
func main() {
	...
	if err != nil {
		switch {
		case errors.Is(err, ErrorEmptySourcePath):
			flag.Usage()
		case errors.Is(err, ErrorEmptyDestinationPath):
			flag.Usage()
		case errors.Is(err, ErrorFileNotFound):
			fmt.Printf("Error: File not exists: %s\n", from)
		case errors.Is(err, ErrorUnableGetStat):
			fmt.Printf("Error: Unable get file stat: %s\n", from)
		case errors.Is(err, ErrorCopyNonRegular):
		...
		case errors.Is(err, ErrorCreateTmpFile):
